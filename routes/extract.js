...
router.post('/:campaignId/:sessionId', requireAuth, async function(req, res) {
  ...
  const userPrompt =
    ...
    '      "prompt": "Detailed image generation prompt describing the SCENE ONLY. Describe composition, lighting, character positions, mood, and any specific visual details from the director\'s instructions. 2-3 sentences. Do NOT mention or name an art style, medium, or rendering technique - the art style is applied separately at image-generation time, so the prompt must stay style-neutral. CRITICAL FOR CHARACTER CONSISTENCY: every time a KNOWN named character appears, refer to them BY THEIR EXACT NAME (e.g. \\"Ruk\\", \\"Zara\\") - never anonymously like \\"a half-orc\\" or \\"the warrior\\". The name tells the system this is a specific recurring character. GROUP REFERENCES — CRITICAL: the same naming rule applies to GROUP terms. NEVER write \\"the party\\", \\"the group\\", \\"the adventurers\\", \\"the heroes\\", \\"the team\\", \\"the companions\\", \\"the fellowship\\", \\"everyone\\", or any other generic group label as if it were a character. These terms produce GENERIC FACES in the rendered image. INSTEAD, look at the transcript context for THIS specific moment and resolve the group term into the EXPLICIT NAMES of the characters who are actually in this panel. For example, do not write \\"the party fights the dragon\\" — write \\"Ruk, Zara, and Thorin fight the dragon\\". If the moment involves only some of the party, only name those who are present. If the transcript is genuinely ambiguous about who is present, use your judgment from earlier context, but ALWAYS resolve group references into named lists. The same applies to plural pronouns like \\"they\\" or \\"them\\" when they refer to a group — resolve to names. CHARACTER DESCRIPTIONS - KEEP THEM LEAN: a reference image supplies each known character\'s permanent appearance (face, build, features), so do NOT re-describe their fixed physical traits at length. For a known character, focus their text on what they are DOING in this panel (pose, action, expression) and any TEMPORARY visible state from the transcript (bloodied, muddy, exhausted, frightened, soaked). A brief identifying tag is fine the first time (\\"Ruk, the half-orc barbarian\\") but keep it short - the image carries the look. Only describe full physical appearance for UNNAMED background figures who are not known characters."\n' +
    '    }\n' +
    '  ]\n' +
    '}';
