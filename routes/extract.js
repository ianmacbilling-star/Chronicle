...
router.post('/:campaignId/:sessionId', requireAuth, async function(req, res) {
  ...
  const userPrompt =
    ...
    '      "shape": "The frame shape for this panel - choose EXACTLY one of: wide, standard, tall, or square. Pick the shape that best fits the scene composition, so the printed graphic novel can vary panel sizes for a dynamic page. Use wide for sweeping establishing shots, landscapes, a row or line of characters, a charge, a chase, or anything broad and panoramic that reads left to right. Use tall for towering or vertical subjects, a figure falling or climbing, a dramatic full-body or full-height reveal, or a tall narrow space. Use square for an intimate close-up on a single face or object, a tight two-shot, or a focused symbolic beat. Use standard for ordinary panels that do not clearly call for another shape - standard should be the most common choice. Do not overuse the dramatic shapes: reserve wide, tall, and square for moments whose composition genuinely benefits, and let the majority of panels stay standard.",\n' +
  ...
  try {
    ...
    if (parsed.moments && parsed.moments.length) {
      ...
      const insert = await db.prepare(
        'INSERT INTO moments (session_id, fork_id, title, description, type, prompt, emphasis, shape, panel_order, created_at, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
      );
      parsed.moments.forEach(function(m, i) {
        insert.run(session.id, dmForkId, m.title, m.description, m.type, m.prompt, m.emphasis || null, (m.shape === 'wide' || m.shape === 'tall' || m.shape === 'square') ? m.shape : 'standard', i, now, req.session.userId);
      });
