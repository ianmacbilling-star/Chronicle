  ...
  try {
    ...
    if (parsed.moments && parsed.moments.length) {
      ...
      parsed.moments.forEach(function(m, i) {
        insert.run(session.id, dmForkId, m.title, m.description, m.type, m.prompt, m.emphasis || null, (['wide','tall','square','panoramic','tower'].indexOf(m.shape) >= 0 ? m.shape : 'standard'), i, now, req.session.userId);
      });
