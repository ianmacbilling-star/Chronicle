...
router.post('/:campaignId/:sessionId', requireAuth, async function(req, res) {
  ...
  const userPrompt =
    ...
    '      "prompt": "Detailed image generation prompt in ' + style + ' style. Describe composition, lighting, character positions, mood, and any specific visual details from the director\'s instructions. 2-3 sentences. IMPORTANT for character consistency: whenever a known character appears, restate their key physical descriptors (hair, build, distinctive features, signature outfit and colors) right in the prompt - do not just use their name, because the image model has no memory between panels."\n' +
    '    }\n' +
    '  ]\n' +
    '}';
