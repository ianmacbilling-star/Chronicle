...
async function serveStoryPage(req, res) {
  ...
  try {
    ...
    const header =
          ...
          '<div style="flex:1;min-width:220px;">' +
            ...
            '<div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center;">' +
              ...
              '<button type="button" id="cmpShare" data-url="' + esc(shareUrl) + '" data-title="' + esc(title) + '" style="display:inline-flex;align-items:center;gap:7px;background:transparent;color:#c9a84c;border:1px solid rgba(201,168,76,0.5);padding:8px 16px;border-radius:6px;font-weight:600;font-size:13px;font-family:inherit;cursor:pointer;" aria-label="Share this story">' +
                ...
                '</svg>Share</button>' +
              // v3.0.982 -- TD-901. ADD TO BOOKSHELF, for a signed-in Gold or Platinum reader who is not the
              // author. Hidden until /api/bookshelf/library-status says so; a signed-out visitor gets a 401
      ...
      'fetch("/api/bookshelf/library-status/"+encodeURIComponent(id)+"?t="+encodeURIComponent(tk),{credentials:"same-origin"}).then(function(r){return r.ok?r.json():null;}).then(function(s){if(!s||!s.found||s.own||!s.canAdd)return;b.style.display="inline-flex";if(s.onShelf)on();}).catch(function(){});'+
    ...
    if (snap && snap.sessions) {
      ...
      html = '<!DOCTYPE html><html lang="en"><head><link rel="stylesheet" href="/css/fonts.css"><meta charset="utf-8" />' +
        '<meta name="viewport" content="width=device-width, initial-scale=1.0" />' + seo +
        '' +
        PWA_HEAD + '<script src="/analytics.js"></script></head><body style="margin:0;background:#0a0806;color:#f0e8d0;min-height:100vh;">' + header +
