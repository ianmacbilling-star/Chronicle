...
var LAYOUT_GOALS = {
  paired: [
    'LAYOUT GOALS (Picture Book) -- the FIRST MOVE for filling white is: SLIDE CONTENT UP.',
    'GROW A PICTURE ONLY AFTER MOVING. Growing fills a page without moving anything, so it is the easy',
    'answer and the wrong first one: it consumes room a slide needed and can never shorten the book.',
    'Grow what is left over once the sliding is done, and only while the image is still smaller than its',
    'natural size (REAL-CELL < fullH) -- growing past that crops the art.',
    '- WHITE SPACE IS THE PROBLEM, not a style. A page holding three inches of a nine-inch box is not a',
    '  finished page. Read the UNDERFULL lines: each names the one legal move and the room available.',
    '- SLIDE CONTENT UP. Bring the next page\'s leading segment onto an underfull page -- bridge text',
    '  with pullLines, a PICTURE with pullPicture. Done repeatedly this walks content up through the book',
    '  and can empty a page entirely, which shortens it. Fewer, fuller pages is the goal.',
    '- A PICTURE MOVES LIKE A PARAGRAPH, and its pic text follows because they share a segment number.',
    '  pullPicture is often the ONLY legal way to fill a page: one holding the tail of some bridge text',
    '  can be joined by that beat\'s picture and by nothing else. Propose it whenever a hint names it.',
    '- A beat SPLIT ACROSS A PAGE BREAK is normal and reads well -- bridge text at the foot of one page,',
    '  its picture at the head of the next. Do not avoid a split that fills a page.',
