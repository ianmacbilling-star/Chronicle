...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .cover-content {
    ...
    /* v3.0.379 -- TEST TRANSPARENCY, VISIBLE IN THE PREVIEW TABS.
       0.6 matches the wrap-cover front logo set in v3.0.378, so one value can be compared
       on screen, in the flattened print PDF, and on the printed proof. NOTE paneSafeHtml
       does NOT touch opacity, so the preview shows what the print HTML actually contains --
       unlike the caption gradients, which the panes flatten for display and which therefore
       cannot test the flattener at all.
       Five sites carry this; revert all five to remove. Diagnostic, not a design decision. */
  .cover-logo {
    width: 160px;
    height: auto;
    object-fit: contain;
  ...
  .cover-art-logo { width:110px;height:auto;object-fit:contain;opacity:0.6; }   /* v3.0.379 test transparency */
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .cover-logo { width:160px;height:auto;object-fit:contain;opacity:0.6;margin-bottom:0.4in; }   /* v3.0.379 test transparency */
  ...
  .dp-logo { width:0.85in;height:auto;object-fit:contain;display:block;margin:0.3in auto 0.12in;opacity:0.6; }   /* v3.0.379 test transparency, was 0.9 */
...
async function _computeMagazinePackInner(req, campaignId, packOpts) {
  ...
  pages.forEach(function (pg, pi) {
    ...
    // v3.0.366 -- this gate's own comment says 'a splittable FLOAT', but it never checked the kind:
    // remeta + stext + simg are all set by mzFeatureBand too, so a MAXIMIZE feature fell straight
    // through it. For ALL the Ages band 21 ('Leap Through the Portal!', prominence max, portrait)
    ...
    if (mul >= _gfFloor && mul < 0.98) { grow[nbi] = Math.round(mul * 100) / 100; splitAllow[nbi] = true; }
  });
...
var LAYOUT_REVIEW_SYSTEM = [
  ...
  '  { "op":"pullPicture", "page":N, "fromPage":N, "why":"..." }',
  '  { "op":"pushPicture", "page":N, "why":"..." }',
  ...
  '',
  'Rules: prefer the FEWEST ops that FULLY resolve real issues -- fewest does NOT mean one per page, and',
  'a page needing a chain of two or three moves to finish should get all of them. Do not invent ops or',
  'fields. Do not propose an',
...
var LAYOUT_GOALS = {
  paired: [
    'LAYOUT GOALS (Picture Book) -- the FIRST MOVE for filling white on this book is: SLIDE CONTENT UP.',
    'Bring the leading segment of the NEXT page onto an underfull page -- bridge text with pullLines, a',
    'PICTURE with pullPicture. A picture moves exactly like a paragraph, and the pic text follows it,',
    'because they share a segment number. Sliding is the only lever that can shorten the book.',
    'CHAIN YOUR MOVES: you may propose SEVERAL moves for the SAME page in one pass. Ops apply IN ORDER',
    'and each one sees the state the previous one left. So pull the bridge text up, and then pull the',
    'PICTURE that is now leading the following page up behind it -- that finishes the page in ONE pass',
    'instead of waiting for the next. A beat is opening text, picture, closing text, so finishing a beat',
    'usually takes TWO OR THREE moves, not one. Keep a running total against the 9.24in box as you chain',
    'and stop when the next segment would not fit.',
    'GROW is the SECOND move, not the first. The picture is the show, so grow its image to fill the',
    'space -- but ONLY if the image is currently smaller than its natural full size (REAL-CELL < fullH).',
    'A full-width image already at natural size cannot grow without cropping the art -- do NOT grow it.',
    'Re-check that every pass: an image an earlier pass SHRANK has headroom again and can be grown back.',
    '- A page holding 8.5in or more of the 9.24in box is FULL. Leave it alone and spend the pass',
    '  on pages with real room.',
    '- SOME WHITE SPACE IS ACCEPTABLE. A full-width image with some white below it is a FINISHED page.',
    '  Leave it alone -- growing it would crop the art, and there may be no legal move that fills it.',
