...
// v3.0.380 -- COVER LOGO OPACITY, TUNABLE WITHOUT A DEPLOY.
// It began as a one-off diagnostic for the v3.0.377 flattener and is now a design
// setting Ian is dialling in, so it stops being a literal pasted into a CSS string.
// Set COVER_LOGO_OPACITY on Railway to try another value -- an env change restarts the
// service rather than rebuilding it, so a round trip is a restart instead of a build.
// 1 turns the effect off entirely, which is also how the test gets retired.
// FRONT COVER ONLY. The spine logo stays at 0.95 so one printed sheet still carries a
// heavy alpha and a light one to compare, and the interior logos are opaque again.
var COVER_LOGO_OPACITY = (function () {
  var v = Number(process.env.COVER_LOGO_OPACITY);
  if (!(v >= 0 && v <= 1)) v = 0.45;
